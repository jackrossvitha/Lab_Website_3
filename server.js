...
app.get('/player_info'){
  var query = 'SELECT id, player_name FROM football_players;';
 db.any(query)
     .then(function (rows) {
...
app.get('/player_info/select_player', function(req, res), player_id){
  var query1 = 'SELECT id, player_name FROM football_players;';
  var query2 = 'select * from football_players where id = ' + player_id + ';';
  var query3 = 'select * from football_games where any(id = ' + player_id + ');';
  db.task('get-everything', task => {
    return task.batch([
        task.any(query1),
        task.any(query2),
